...
# --- THE PROXY POOL ---
# Automatically gathers up to 5 proxies from GitHub Secrets
PROXIES = []
for i in range(1, 6):
...
# Keeps your old secret name working just in case
...
def process_channel(channel):
    ...
    if not result and PROXIES:
        for index, proxy in enumerate(PROXIES, start=1):
            try:
                ...
                if result:
                    break # Success! Break out of the loop and stop burning other proxies
            except Exception:
                print(f"  -> Proxy {index} failed/exhausted. Moving to next...")
                result = None
    ...
    if not result:
        try:
            ...
            print(f"  -> FATAL: All proxies & routes failed for {channel['id']} 🔴")
            return None
...
def update_playlist():
    ...
    print(f"Base playlist saved. Spawning Smart Extractors with {len(PROXIES)} active proxies...")
